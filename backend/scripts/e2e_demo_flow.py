...
# ---------------------------------------------------------------- ⑤ 模拟下单
step(5, "模拟预约下单（选套餐/房型 → 填写姓名手机号 → 选择时间 → 提交）")
before = call("/api/orders")["data"]["total"]
pkg = shop["packages"][1]
created = call("/api/orders", "POST", {
    "type": "shop", "targetId": "S001", "itemId": pkg["id"],
    "name": "现场演示", "phone": "13800001234", "bookTime": "2026-10-20", "count": 2,
})["data"]
ok(f"门店套餐下单成功 → 订单号 {created['orderNo']}，{pkg['name']} × 2 = ¥{created['amount']}（状态：{created['status']}）")
room = hotel["rooms"][0]
...
after = call("/api/orders")["data"]["total"]
ok(f"内存订单数 {before} → {after}（下单已追加至后端内存集合）")

# ---------------------------------------------------------------- ⑥ 个人中心
step(6, "个人中心（订单列表 / 订单详情 / 模拟取消预约）")
orders = call("/api/orders")["data"]
ok(f"订单列表共 {orders['total']} 条（预置 3 条 + 本次演示新增）")
...
detail = call(f"/api/orders/{created['orderNo']}")["data"]
ok(f"订单详情：{detail['targetName']}｜{detail['itemName']}｜预约人 {detail['name']}｜¥{detail['amount']}｜{detail['status']}")
cancel_res = call(f"/api/orders/{created['orderNo']}/cancel", "POST")["data"]
ok(f"模拟取消预约 → {cancel_res['orderNo']} 状态变为「{cancel_res['status']}」")
recheck = call(f"/api/orders/{created['orderNo']}")["data"]
...
# ---------------------------------------------------------------- ⑦ 原型限制
step(7, "原型限制说明（答辩口径）")
ok("数据源为后端内存集合，未做数据库持久化：后端服务重启后，本次新增订单与取消状态全部丢失")
ok("无用户注册登录、无商家端与管理后台、无真实支付与消息通知，仅面向 C 端浏览用户的演示流程")
ok("演示中途不要重启后端，否则演示中新建的订单会消失")
